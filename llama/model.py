...
def is_model_parallel_initialized():
    ...
    if not FAIRSCALE_AVAILABLE or not torch.distributed.is_initialized():
        return False
    return fs_init.model_parallel_is_initialized()
...
class Attention(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.use_parallel = is_model_parallel_initialized()
    ...
    def forward(
        ...
        # Conditionally use the KV cache based on the model's mode (training or eval)
        if self.training:
            # During training, we don't use the KV cache.
            keys, values = xk, xv
        else:
            # During inference, we use the KV cache.
...
# TransformerBlock h.shape torch.Size([1, 50, 4096])  out.shape torch.Size([1, 50, 4096])
